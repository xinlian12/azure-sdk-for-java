...
| **Resource leak** | `ClientTelemetry.close()` is a **no-op** — leaks IMDS HTTP client pool per client |
...
| Max size | **Unbounded** ⚠️ — no size limit, no LRU, no TTL |
| Per-client? | ✅ Yes |
| Eviction | None |

**Per-entry**: Key (SQL query text ~100 chars) ≈ 256 B + Value (`PartitionedQueryExecutionInfo` with Jackson `ObjectNode`) ≈ 200–2000 B + Node overhead ≈ 32 B → **~500 B – 2.5 KB per entry**

| Scenario | Entries | Memory |
|---|---|---|
| Typical (100 queries) | 100 | **~100 KB** |
| Heavy (500 queries) | 500 | **~1 MB** |
| Worst case (10K+ queries, long-running) | 10,000+ | **~10–25 MB** ⚠️ |
...
**Source**: `ClientTelemetryInfo.java`

```java
private Map<ReportPayload, ConcurrentDoubleHistogram> systemInfoMap;       // CPU + Memory
...
All three are unbounded `ConcurrentHashMap` instances.

**Per-histogram**: `ConcurrentDoubleHistogram` with precision=4 → internal `long[]` ≈ **~256 KB** (latency histograms). Request charge histograms (precision=2) ≈ **~4 KB**.

| State | Memory |
|---|---|
| Idle | **~50 KB** |
| Active (10 operation types) | **~1–3 MB** (latency histograms dominate) |

Additionally, `ClientTelemetry` creates a **dedicated IMDS HTTP client pool** (5 connections) for Azure VM metadata queries.
...
```java
// IMDS HTTP client with 5-connection pool
this.metadataHttpClient = getHttpClientForIMDS(configs);
```

| Property | Value |
|---|---|
| IMDS pool | **5-connection** `ConnectionProvider` per client |
| close() | ⚠️ **NO-OP** — see [Bug #1](#bug-1-clienttelemetryclose-is-a-no-op) |
...
| **Telemetry histograms** | ~100 KB idle | **~1–3 MB** peak | `ClientTelemetryInfo` — `ConcurrentDoubleHistogram` |
| **Connection pool + active conns** | ~320 KB | **~2–4 MB** (50 active × 32–64 KB) | Reactor Netty channel + SSL |
| **IMDS HTTP client pool** | ~50 KB | ~50 KB | `ClientTelemetry.getHttpClientForIMDS()` — 5 connections |
...
### Bug #1: `ClientTelemetry.close()` Is a No-Op
...
```java
public void close() {
    logger.debug("GlobalEndpointManager closed.");  // ← wrong message, wrong class name
}
```

`close()` only logs a misleading debug message ("GlobalEndpointManager closed." — it's `ClientTelemetry`!) and does **NOT**:
- Close `metadataHttpClient` (IMDS HTTP client with 5-connection pool)
- Clear maps in `clientTelemetryInfo`
- Cancel any ongoing operations
- Set any `isClosed` flag

**Additionally**, `RxDocumentClientImpl.close()` **never calls** `this.clientTelemetry.close()` — so even if `close()` were properly implemented, it wouldn't be invoked.

**Impact per client lifecycle**: Leaks:
- 1 IMDS `ConnectionProvider` with 5-connection pool
- `ClientTelemetryInfo` histograms (up to 3 MB)
- References preventing GC of dead client objects
...
### Bug #3: `GlobalPartitionEndpointManagerForCircuitBreaker` Inconsistency

**Severity**: 🟡 LOW

The regular `globalPartitionEndpointManagerForCircuitBreaker` is only closed if it's **not null** (the `if` check exists), but the other `globalPartitionEndpointManagerForPerPartitionCircuitBreaker` is closed. The naming suggests there may be two separate objects, and both should be closed.
...
|---|---|---|---|
| **A1** | Fix `ClientTelemetry.close()` — properly close `metadataHttpClient`, clear `clientTelemetryInfo` maps, set `isClosed` flag, fix log message | 🔴 P0 | `ClientTelemetry.java` |
| **A2** | Call `this.clientTelemetry.close()` from `RxDocumentClientImpl.close()` | 🔴 P0 | `RxDocumentClientImpl.java` |
...
| # | Action | Benefit |
|---|---|---|
| **A7** | Add LRU eviction to `queryPlanCache` (Caffeine or bounded `LinkedHashMap`) | Prevents unbounded growth |
