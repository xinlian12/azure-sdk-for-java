...
object SampleE2EInnerJoinITest {
    def main(args: Array[String]) {
        ...
        val dfWithCosmosIdentity =
            dfWithLimit10
                .withColumn("cosmosIdentity", functions.concat(dfWithLimit10("id"), lit(" "), dfWithLimit10("id")))
...
//        df.createOrReplaceTempView("cosmosView")
        ...
        val result = df.join(dfWithCosmosIdentity).where(df("id") === dfWithCosmosIdentity("cosmosIdentity") && dfWithCosmosIdentity("vendorIDInc") >= 2).collect()
