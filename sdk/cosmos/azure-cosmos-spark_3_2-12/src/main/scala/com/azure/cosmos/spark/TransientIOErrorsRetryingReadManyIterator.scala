...
class TransientIOErrorsRetryingReadManyIterator[TSparkRow](
                                                   ...
                                                   val cosmosItemIdentityList: List[CosmosItemIdentity],
    ...
    private[spark] val cosmosItemIdentityBatchIterator = cosmosItemIdentityList.grouped(pageSize)
    ...
    private def hasNextInternalCore: Option[Boolean] = {
        if (hasBufferedNext) {
            ...
            if (cosmosItemIdentityBatchIterator.hasNext) {
                val cosmosItemIdentityBatch = cosmosItemIdentityBatchIterator.next()
                val feedResponse = container.readMany(cosmosItemIdentityBatch.asJava, queryOptions, classType).block()
