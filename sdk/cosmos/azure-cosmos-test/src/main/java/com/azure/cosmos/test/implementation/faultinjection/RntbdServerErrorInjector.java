...
public class RntbdServerErrorInjector implements IRntbdServerErrorInjector {
    ...
    public boolean injectRntbdServerResponseError(RntbdRequestRecord requestRecord) {
        ...
        FaultInjectionServerErrorRule serverResponseErrorRule = this.ruleStore.findRntbdServerResponseErrorRule(requestArgs);
    ...
    public boolean injectRntbdServerConnectionDelay(
        ...
        RntbdRequestArgs rntbdRequestArgs = requestRecord.args();
        FaultInjectionServerErrorRule serverConnectionDelayRule = this.ruleStore.findRntbdServerConnectionDelayRule(rntbdRequestArgs);

        if (serverConnectionDelayRule != null) {
            rntbdRequestArgs.serviceRequest().faultInjectionRequestContext
                .applyFaultInjectionRule(
                    requestRecord.transportRequestId(),
